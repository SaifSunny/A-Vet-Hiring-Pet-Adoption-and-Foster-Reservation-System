    ...
    if ($password == $cpassword) {
            if (strlen($p) > 5) {
                ...
                if (!$query_run->num_rows > 0) {
                    $query = "SELECT * FROM admin WHERE adminname = '$adminname' AND email = '$email'";
                    $query_run = mysqli_query($conn, $query);

                    if(!$query_run->num_rows > 0){
                        $query2 = "INSERT INTO admin(adminname,email,`password`)
        ...
        <div class="container">
            <form action="" method="POST" class="login-email">
                ...
                <div class="input-group">
                    <input type="text" placeholder="adminname" name="adminname" value="<?php echo $adminname; ?>" required>
                </div>
                <div class="input-group">
