...
if (isset($_POST['submit'])) {
	...
	if ($result->num_rows > 0) {
        ...
        if ($result->num_rows > 0) {
            $sql = "SELECT * FROM vets WHERE vetname='$vetname' AND password='$password'";
            $result = mysqli_query($conn, $sql);
        
            if ($result->num_rows > 0) {
                $_SESSION['vetname'] = $_POST['vetname'];

                $sql = "INSERT INTO recent(`image`, `name`, `role`) VALUES ((SELECT `image` FROM vets WHERE vetname='$vetname'), '$vetname', 'Vet')";
                $result = mysqli_query($conn, $sql);
                if($result){
                    header("Location: vet_home.php");
                }
        ...
        <div class="container">
            <form action="" method="POST" class="login-email">
                ...
                <div class="input-group"  style="color:black;">
                    <input type="text" placeholder="Username" name="vetname" value="" required>
                </div>
                <div class="input-group"  style="color:black;">
