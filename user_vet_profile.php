...
if(isset($_POST['submit'])){
    ...
    $query = "SELECT * FROM vet_appointment WHERE user_id = '$user_id' AND vet_id = '$vet_id' AND `date` = '$date' AND `time` = '$time'";
    ...
    if(!$query_run->num_rows > 0){
        ...
        if(!$query_run->num_rows > 0){
            ...
            VALUES ('$user_id', '$vet_id', '$user_img', (SELECT `image` FROM vets WHERE vet_id='$vet_id'), '$username', (SELECT CONCAT(firstname,' ',lastname) FROM vets WHERE vet_id='$vet_id'), '$pname', '$date', '$time', '$contact','$email', '$message')";
