    ...
    if ($password == $cpassword) {
            if (strlen($p) > 5) {
                ...
                if (!$query_run->num_rows > 0) {
                    $query = "SELECT * FROM vets WHERE vetname = '$vetname' AND email = '$email'";
                    $query_run = mysqli_query($conn, $query);

                    if(!$query_run->num_rows > 0){
                        $query2 = "INSERT INTO vets(vetname,email,`password`)
                        VALUES ('$vetname', '$email', '$password')";
        ...
        <div class="container">
            <form action="" method="POST" class="login-email">
                ...
                <div class="input-group">
                    <input type="text" placeholder="Username" name="vetname" value="<?php echo $vetname; ?>" required>
                </div>
                <div class="input-group">
