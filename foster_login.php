...
if (isset($_POST['submit'])) {
	...
	if ($result->num_rows > 0) {
        ...
        $sql = "SELECT * FROM fosters WHERE `password`='$password'";
        $result = mysqli_query($conn, $sql);
    
        if ($result->num_rows > 0) {
            $sql = "SELECT * FROM fosters WHERE fostername='$fostername' AND password='$password'";
            $result = mysqli_query($conn, $sql);
        
            if ($result->num_rows > 0) {
                $_SESSION['fostername'] = $_POST['fostername'];

                $sql = "INSERT INTO recent(`image`, `name`, `role`) VALUES ((SELECT `image` FROM fosters WHERE fostername='$fostername'), '$fostername', 'Foster')";
                $result = mysqli_query($conn, $sql);
                if($result){
                    header("Location: foster_home.php");
                }
        ...
        <div class="container">
            <form action="" method="POST" class="login-email">
                ...
                <div class="input-group"  style="color:black;">
                    <input type="text" placeholder="Username" name="fostername" value="" required>
                </div>
                <div class="input-group"  style="color:black;">
                ...
                <p class="login-register-text">Don't have an account? <a href="foster_register.php">Register Here</a>.</p>
            </form>
        </div>
